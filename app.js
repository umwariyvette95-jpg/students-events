  ...
  for (let i = 0; i < list.length; i++) {
    ...
    card.innerHTML = `
      ...
      <div class="w-full bg-gray-100 rounded-full h-2 mb-1 overflow-hidden">
        <div class="h-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-300"
      ...
      <div class="flex gap-2 mb-4">
        <div class="flex-1 bg-gray-50 rounded-xl p-2 text-center">
          ...
          <span class="block text-xs text-gray-400 font-semibold">Total</span>
        </div>
        <div class="flex-1 bg-gray-50 rounded-xl p-2 text-center">
          <span class="block text-lg font-extrabold">${event.registered}</span>
          <span class="block text-xs text-gray-400 font-semibold">Registered</span>
        </div>
        <div class="flex-1 bg-green-50 rounded-xl p-2 text-center">
          ...
          <span class="block text-xs text-gray-400 font-semibold">Left</span>
        </div>
      </div>

      <div class="flex gap-2">
        <button onclick="register(${event.id})"
          class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-xl transition-colors">
          Register
        </button>
        <button onclick="cancelRegistration(${event.id})"
          class="flex-1 bg-red-50 hover:bg-red-100 text-red-500 font-bold py-2 rounded-xl transition-colors">
          Cancel
        </button>
      </div>
    `;
...
function updateStats() {
  ...
  for (let i = 0; i < events.length; i++) {
    registered = registered + events[i].registered;
    remaining  = remaining  + (events[i].seats - events[i].registered);
  }
